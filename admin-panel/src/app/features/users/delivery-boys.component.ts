...
export class DeliveryBoysComponent implements OnInit {
  ...
  load(page = this.currentPage()) {
    this.api.get<any>('/admin/delivery-boys', { page, size: 20 }).subscribe((res) => {
  ...
  create() {
    ...
    this.api.post('/admin/delivery-boys', this.form.getRawValue()).subscribe(() => {
      this.form.reset({ fullName: '', phone: '' });
      this.load();
      this.snack.open('Delivery boy created', 'OK', { duration: 1600 });
    });
  }
  ...
  toggle(user: any) {
    this.api.patch(`/admin/users/${user.id}/active`, { active: !user.active }).subscribe(() => {
      this.load();
      this.snack.open('Delivery boy status updated', 'OK', { duration: 1600 });
    });
  }
}
